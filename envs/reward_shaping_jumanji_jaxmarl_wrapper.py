...
class RewardShapingJumanjiToJaxMARL(JumanjiToJaxMARL):
    ...
    def __init__(self, env, reward_shaping_params=None):
        ...
        self.reward_shaping_params = (
            {
                agent_id: dict(REWARD_SHAPING_PARAMS)
    ...
    def step(self, key, state: RewardShapingEnvState, actions, params=None):
        ...
        shaped_rewards_dict = self._extract_shaped_rewards(obs, state.prev_env_state, state.env_state, actions)
        total_reward_dict = {
            agent: reward[agent] + shaped_rewards_dict[agent]
        ...
        return obs, next_state, total_reward_dict, done, new_info
