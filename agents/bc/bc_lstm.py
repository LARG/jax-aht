...
class LBFBCProxyPartnerWrapper(AgentPolicy):
    """Adapter that lets the LBF BC-LSTM run as a frozen partner inside
    ppo_br training (which routes per-env partners through
    `HeuristicPolicyPopulation`).

    BCLSTMPolicyWrapper expects leading-batch hstate `((B, D), (B, D))` and
    works with batched obs. HeuristicPolicyPopulation vmaps over an envs axis,
    so per-call inputs have no leading batch (hstate `(D,)` per-element, obs
    `(1, obs_dim)`). This wrapper bridges:
      - bakes BC params in at construction so callers can pass `params=None`,
      - returns per-element `(D,)` hstate from `init_hstate`,
      - adds/removes the leading batch dim around the wrapped `get_action`.
    """

    def __init__(self, bc_wrapper: 'BCLSTMPolicyWrapper', bc_params):
        super().__init__(action_dim=bc_wrapper.action_dim, obs_dim=bc_wrapper.obs_dim)
        self.bc = bc_wrapper
        self._params = bc_params

    def init_hstate(self, batch_size, aux_info=None):
        D = self.bc.config.lstm_dim
        return (jnp.zeros((D,)), jnp.zeros((D,)))

    @partial(jax.jit, static_argnums=(0,))
    def get_action(self, params, obs, done, avail_actions, hstate, rng,
                   env_state=None, aux_obs=None, test_mode=False):
        # Add a leading batch dim to hstate so BCLSTMPolicyWrapper sees its (B, D) form.
        h_b = (hstate[0][jnp.newaxis, ...], hstate[1][jnp.newaxis, ...])
        action, new_h_b = self.bc.get_action(
            params=self._params,        # frozen baked-in params; ignore caller's
            obs=obs,
            done=done,
        ...
        return action.squeeze(), (new_h_b[0][0], new_h_b[1][0])
