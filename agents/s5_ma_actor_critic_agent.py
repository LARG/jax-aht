...
from agents.s5_ma_actor_critic import S5Actor, S5Critic, StackedEncoderModel, init_S5SSM, make_DPLR_HiPPO


class S5MAActorCriticPolicy(AgentPolicy):
    ...
    def get_action(self, params, obs, done, avail_actions, hstate, rng,
        ...
        new_hstate, pi, _ = self.actor.apply(params, hstate, (obs, done, avail_actions))
    ...
    def get_action_value_policy(self, params, state, obs, done, avail_actions, hstate, rng,
        ...
        """
        new_actor_hstate, pi = self.actor.apply(params[0], hstate[0], (obs, done, avail_actions))
        action = pi.sample(seed=rng)

        new_critic_hstate, val = self.critic.apply(params[1], hstate[1].squeeze(0), (state, done))

        return action, val, pi, new_actor_hstate, new_critic_hstate

    def init_hstate(self, batch_size, aux_info=None):
        """Initialize hidden state for the S5 policy."""

        init_actor_hstate =  StackedEncoderModel.initialize_carry(batch_size, self.ssm_size // 2, self.ssm_n_layers)
        return init_actor_hstate

    def init_critic_hstate(self, batch_size, aux_info=None):
        """Initialize hidden state for the S5 policy."""

        init_critic_hstate =  StackedEncoderModel.initialize_carry(batch_size, self.ssm_size // 2, self.ssm_n_layers)
        return init_critic_hstate
