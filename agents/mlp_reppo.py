...
class Actor(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: tuple[jnp.ndarray, jnp.ndarray], train: bool, temperature: float = 1.0):
        self.param(
        ...
        self.param(
            "log_lagrangian",
            nn.initializers.constant(jnp.log(self.init_lagrangian)),
            (1,),
        )
        ...
        x = x - (unavail_actions * 1e10)

        return distrax.Categorical(logits=x / temperature)
