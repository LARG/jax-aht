...
class S5Critic(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        if self.s5_no_reset:
            dones = jnp.zeros_like(dones)
        embedding = self.encoder_0(state)
        embedding = nn.leaky_relu(embedding)
        embedding = self.encoder_1(embedding)
        embedding = nn.leaky_relu(embedding)
        ...
        for layer in self.value_body_layers:
            ...
            value_out = nn.leaky_relu(value_out)
        value_out = self.value_decoder(value_out)

        return hidden, jnp.squeeze(value_out, axis=-1)
