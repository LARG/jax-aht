...
def _bootstrap_one(args):
    ...
    aggregate_stat_fn = (
        rli_metrics.aggregate_iqm
        if agg_stat_name == "iqm"
        else rli_metrics.aggregate_mean
    )
...
def compute_aggregate_stat_and_ci(
    data: np.ndarray, agg_stat_name: str, return_interval_est: bool
):
...
def compute_aggregate_stat_and_ci_per_task(
    ...
    """
    assert data.ndim == 2, "Data must be 2D."
    _num_runs, num_tasks = data.shape
    aggregate_stat_fn = get_aggregate_stat_fn(agg_stat_name)
