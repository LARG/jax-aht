...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="neurips_eval_variance.html",
    ...
    parser.add_argument(
        "--data-cache",
        type=str,
        default="plotting_data/neurips_eval_cache.pkl",
...
def main() -> None:
    ...
    algos = ['IPPO', 'IPPO (Centralized)', 'MAPPO (Centralized)', 'PPO']
    ...
    #     ippo_filters.append({
    #         "tags": {
    #             "$all": [agent, "marl_comparison", "ippo"],
    #             "$nin": ["OLD", "centralized", "social law", "social law yielding", "curriculum"],
    #         }
    #     })

    ippo_centralized_filters = []
    for agent in agents:
        ippo_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ippo", "centralized"],
                "$nin": ["OLD", "social law", "social law yielding", "curriculum"],
            }
        })
    ...
    #         "tags": {
    #             "$all": [agent, "marl_comparison", "ippo", "social law"],
    #             "$nin": ["OLD", "centralized", "social law yielding", "curriculum"],
    #         }
    #     })
    ...
    for agent in agents:
        ippo_social_law_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ippo", "social law", "centralized"],
                "$nin": ["OLD", "social law yielding", "curriculum"],
            }
        })
    ...
    #         "tags": {
    #             "$all": [agent, "marl_comparison", "ippo", "social law yielding"],
    #             "$nin": ["OLD", "centralized", "social law", "curriculum"],
    #         }
    #     })
    ...
    for agent in agents:
        ippo_social_law_yielding_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ippo", "social law yielding", "centralized"],
                "$nin": ["OLD", "social law", "curriculum"],
            }
        })

    # IPPO Curriculum variants
    # ippo_curriculum_social_law_filters = []
    # for agent in agents:
    #     ippo_curriculum_social_law_filters.append({
    #         "tags": {
    ...
    ippo_curriculum_social_law_centralized_filters = []
    for agent in agents:
        ippo_curriculum_social_law_centralized_filters.append({
            "tags": {
    ...
    # ippo_curriculum_social_law_yielding_filters = []
    # for agent in agents:
    #     ippo_curriculum_social_law_yielding_filters.append({
    #         "tags": {
    ...
    ippo_curriculum_social_law_yielding_centralized_filters = []
    for agent in agents:
        ippo_curriculum_social_law_yielding_centralized_filters.append({
            "tags": {
    ...
    # ippo_runs = []
    # for ippo_filter in ippo_filters:
    #     ippo_runs.append(api.runs(path=path, filters=ippo_filter, per_page=args.max_runs))

    ippo_centralized_runs = []
    for ippo_centralized_filter in ippo_centralized_filters:
        ippo_centralized_runs.append(api.runs(path=path, filters=ippo_centralized_filter, per_page=args.max_runs))
    ...
    # ippo_curriculum_social_law_runs = []
    # for ippo_curriculum_social_law_filter in ippo_curriculum_social_law_filters:
    #     ippo_curriculum_social_law_runs.append(api.runs(path=path, filters=ippo_curriculum_social_law_filter, per_page=args.max_runs))

    ippo_curriculum_social_law_centralized_runs = []
    for ippo_curriculum_social_law_centralized_filter in ippo_curriculum_social_law_centralized_filters:
        ippo_curriculum_social_law_centralized_runs.append(api.runs(path=path, filters=ippo_curriculum_social_law_centralized_filter, per_page=args.max_runs))

    # ippo_curriculum_social_law_yielding_runs = []
    # for ippo_curriculum_social_law_yielding_filter in ippo_curriculum_social_law_yielding_filters:
    #     ippo_curriculum_social_law_yielding_runs.append(api.runs(path=path, filters=ippo_curriculum_social_law_yielding_filter, per_page=args.max_runs))

    ippo_curriculum_social_law_yielding_centralized_runs = []
    for ippo_curriculum_social_law_yielding_centralized_filter in ippo_curriculum_social_law_yielding_centralized_filters:
        ippo_curriculum_social_law_yielding_centralized_runs.append(api.runs(path=path, filters=ippo_curriculum_social_law_yielding_centralized_filter, per_page=args.max_runs))

    # MAPPO (Centralized)
    mappo_centralized_filters = []
    for agent in agents:
        mappo_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "mappo", "centralized"],
                "$nin": ["OLD", "social law", "social law yielding", "curriculum"],
            }
        })
    ...
    for agent in agents:
        mappo_social_law_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "mappo", "social law", "centralized"],
                "$nin": ["OLD", "social law yielding", "curriculum"],
            }
        })
    ...
    for agent in agents:
        mappo_social_law_yielding_centralized_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "mappo", "social law yielding", "centralized"],
                "$nin": ["OLD", "social law", "curriculum"],
            }
        })

    # MAPPO Curriculum variants
    mappo_curriculum_social_law_centralized_filters = []
    for agent in agents:
        mappo_curriculum_social_law_centralized_filters.append({
            "tags": {
    ...
    mappo_curriculum_social_law_yielding_centralized_filters = []
    for agent in agents:
        mappo_curriculum_social_law_yielding_centralized_filters.append({
            "tags": {
    ...
    mappo_centralized_runs = []
    for mappo_centralized_filter in mappo_centralized_filters:
        mappo_centralized_runs.append(api.runs(path=path, filters=mappo_centralized_filter, per_page=args.max_runs))
    ...
    mappo_curriculum_social_law_centralized_runs = []
    for mappo_curriculum_social_law_centralized_filter in mappo_curriculum_social_law_centralized_filters:
        mappo_curriculum_social_law_centralized_runs.append(api.runs(path=path, filters=mappo_curriculum_social_law_centralized_filter, per_page=args.max_runs))

    mappo_curriculum_social_law_yielding_centralized_runs = []
    for mappo_curriculum_social_law_yielding_centralized_filter in mappo_curriculum_social_law_yielding_centralized_filters:
        mappo_curriculum_social_law_yielding_centralized_runs.append(api.runs(path=path, filters=mappo_curriculum_social_law_yielding_centralized_filter, per_page=args.max_runs))
    ...
    for agent in agents:
        ppo_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ppo"],
                "$nin": ["OLD", "social law", "social law yielding"],
            }
        })

    ppo_social_law_filters = []
    for agent in agents:
        ppo_social_law_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ppo", "social law"],
                "$nin": ["OLD", "social law yielding"],
            }
        })

    ppo_social_law_yielding_filters = []
    for agent in agents:
        ppo_social_law_yielding_filters.append({
            "tags": {
                "$all": [agent, "marl_comparison", "ppo", "social law yielding"],
                "$nin": ["OLD", "social law"],
            }
        })

    ppo_runs = []
    for ppo_filter in ppo_filters:
        ppo_runs.append(api.runs(path=path, filters=ppo_filter, per_page=args.max_runs))

    ppo_social_law_runs = []
    for ppo_social_law_filter in ppo_social_law_filters:
        ppo_social_law_runs.append(api.runs(path=path, filters=ppo_social_law_filter, per_page=args.max_runs))

    ppo_social_law_yielding_runs = []
    for ppo_social_law_yielding_filter in ppo_social_law_yielding_filters:
        ppo_social_law_yielding_runs.append(api.runs(path=path, filters=ppo_social_law_yielding_filter, per_page=args.max_runs))


    for i in range(len(agents)):
        # print(f"IPPO {agents[i]} runs: {len(ippo_runs[i])}")
        print(f"IPPO (Centralized) {agents[i]} runs: {len(ippo_centralized_runs[i])}")
        ...
        # print(f"IPPO (Curriculum - Social Law) {agents[i]} runs: {len(ippo_curriculum_social_law_runs[i])}")
        print(f"IPPO (Curriculum - Social Law + Centralized) {agents[i]} runs: {len(ippo_curriculum_social_law_centralized_runs[i])}")
        # print(f"IPPO (Curriculum - Social Law Yielding) {agents[i]} runs: {len(ippo_curriculum_social_law_yielding_runs[i])}")
        print(f"IPPO (Curriculum - Social Law Yielding + Centralized) {agents[i]} runs: {len(ippo_curriculum_social_law_yielding_centralized_runs[i])}")

        print(f"MAPPO (Centralized) {agents[i]} runs: {len(mappo_centralized_runs[i])}")
        ...
        print(f"MAPPO (Curriculum - Social Law + Centralized) {agents[i]} runs: {len(mappo_curriculum_social_law_centralized_runs[i])}")
        print(f"MAPPO (Curriculum - Social Law Yielding + Centralized) {agents[i]} runs: {len(mappo_curriculum_social_law_yielding_centralized_runs[i])}")

        print(f"PPO {agents[i]} runs: {len(ppo_runs[i])}")
        print(f"PPO (Social Law) {agents[i]} runs: {len(ppo_social_law_runs[i])}")
        print(f"PPO (Social Law Yielding) {agents[i]} runs: {len(ppo_social_law_yielding_runs[i])}")
    ...
    if args.data_cache and os.path.exists(args.data_cache) and not args.refresh_data_cache:
        ...
        # ippo_returns_by_agent = cached.get("ippo_returns_by_agent")
        # ippo_collisions_by_agent = cached.get("ippo_collisions_by_agent")
        ippo_centralized_returns_by_agent = cached.get("ippo_centralized_returns_by_agent")
        ippo_centralized_collisions_by_agent = cached.get("ippo_centralized_collisions_by_agent")
        ...
        # ippo_curriculum_social_law_returns_by_agent = cached.get("ippo_curriculum_social_law_returns_by_agent")
        # ippo_curriculum_social_law_collisions_by_agent = cached.get("ippo_curriculum_social_law_collisions_by_agent")
        ippo_curriculum_social_law_centralized_returns_by_agent = cached.get("ippo_curriculum_social_law_centralized_returns_by_agent")
        ippo_curriculum_social_law_centralized_collisions_by_agent = cached.get("ippo_curriculum_social_law_centralized_collisions_by_agent")
        # ippo_curriculum_social_law_yielding_returns_by_agent = cached.get("ippo_curriculum_social_law_yielding_returns_by_agent")
        # ippo_curriculum_social_law_yielding_collisions_by_agent = cached.get("ippo_curriculum_social_law_yielding_collisions_by_agent")
        ippo_curriculum_social_law_yielding_centralized_returns_by_agent = cached.get("ippo_curriculum_social_law_yielding_centralized_returns_by_agent")
        ippo_curriculum_social_law_yielding_centralized_collisions_by_agent = cached.get("ippo_curriculum_social_law_yielding_centralized_collisions_by_agent")
        mappo_centralized_returns_by_agent = cached.get("mappo_centralized_returns_by_agent")
        mappo_centralized_collisions_by_agent = cached.get("mappo_centralized_collisions_by_agent")
        ...
        mappo_curriculum_social_law_centralized_returns_by_agent = cached.get("mappo_curriculum_social_law_centralized_returns_by_agent")
        mappo_curriculum_social_law_centralized_collisions_by_agent = cached.get("mappo_curriculum_social_law_centralized_collisions_by_agent")
        mappo_curriculum_social_law_yielding_centralized_returns_by_agent = cached.get("mappo_curriculum_social_law_yielding_centralized_returns_by_agent")
        mappo_curriculum_social_law_yielding_centralized_collisions_by_agent = cached.get("mappo_curriculum_social_law_yielding_centralized_collisions_by_agent")
        ppo_returns_by_agent = cached.get("ppo_returns_by_agent")
        ppo_collisions_by_agent = cached.get("ppo_collisions_by_agent")
        ppo_social_law_returns_by_agent = cached.get("ppo_social_law_returns_by_agent")
        ppo_social_law_collisions_by_agent = cached.get("ppo_social_law_collisions_by_agent")
        ppo_social_law_yielding_returns_by_agent = cached.get("ppo_social_law_yielding_returns_by_agent")
        ppo_social_law_yielding_collisions_by_agent = cached.get("ppo_social_law_yielding_collisions_by_agent")
        loaded_cache = True

    if not loaded_cache:
        # ippo_returns_by_agent, ippo_collisions_by_agent = collect_metric_series(
        # run_collections=ippo_runs,
        # agent_labels=agents,
        # returns_metric="Eval/Return",
        # collisions_metric="Eval/Collisions",
        # max_workers=args.max_workers,
        # )
        # print_collection_summary("IPPO", ippo_returns_by_agent, ippo_collisions_by_agent)
        ippo_centralized_returns_by_agent, ippo_centralized_collisions_by_agent = collect_metric_series(
            run_collections=ippo_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("IPPO (Centralized)", ippo_centralized_returns_by_agent, ippo_centralized_collisions_by_agent)
        ...
        mappo_centralized_returns_by_agent, mappo_centralized_collisions_by_agent = collect_metric_series(
            run_collections=mappo_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("MAPPO (Centralized)", mappo_centralized_returns_by_agent, mappo_centralized_collisions_by_agent)
        ...
        # ippo_curriculum_social_law_returns_by_agent, ippo_curriculum_social_law_collisions_by_agent = collect_metric_series(
        #     run_collections=ippo_curriculum_social_law_runs,
        #     agent_labels=agents,
        #     returns_metric="Eval/Return",
        #     collisions_metric="Eval/Collisions",
        #     max_workers=args.max_workers,
        # )
        # print_collection_summary("IPPO (Curriculum - Social Law)", ippo_curriculum_social_law_returns_by_agent, ippo_curriculum_social_law_collisions_by_agent)
        ippo_curriculum_social_law_centralized_returns_by_agent, ippo_curriculum_social_law_centralized_collisions_by_agent = collect_metric_series(
            run_collections=ippo_curriculum_social_law_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("IPPO (Curriculum - Social Law + Centralized)", ippo_curriculum_social_law_centralized_returns_by_agent, ippo_curriculum_social_law_centralized_collisions_by_agent)
        # ippo_curriculum_social_law_yielding_returns_by_agent, ippo_curriculum_social_law_yielding_collisions_by_agent = collect_metric_series(
        #     run_collections=ippo_curriculum_social_law_yielding_runs,
        #     agent_labels=agents,
        #     returns_metric="Eval/Return",
        #     collisions_metric="Eval/Collisions",
        #     max_workers=args.max_workers,
        # )
        # print_collection_summary("IPPO (Curriculum - Social Law Yielding)", ippo_curriculum_social_law_yielding_returns_by_agent, ippo_curriculum_social_law_yielding_collisions_by_agent)
        ippo_curriculum_social_law_yielding_centralized_returns_by_agent, ippo_curriculum_social_law_yielding_centralized_collisions_by_agent = collect_metric_series(
            run_collections=ippo_curriculum_social_law_yielding_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("IPPO (Curriculum - Social Law Yielding + Centralized)", ippo_curriculum_social_law_yielding_centralized_returns_by_agent, ippo_curriculum_social_law_yielding_centralized_collisions_by_agent)
        mappo_curriculum_social_law_centralized_returns_by_agent, mappo_curriculum_social_law_centralized_collisions_by_agent = collect_metric_series(
            run_collections=mappo_curriculum_social_law_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("MAPPO (Curriculum - Social Law + Centralized)", mappo_curriculum_social_law_centralized_returns_by_agent, mappo_curriculum_social_law_centralized_collisions_by_agent)
        mappo_curriculum_social_law_yielding_centralized_returns_by_agent, mappo_curriculum_social_law_yielding_centralized_collisions_by_agent = collect_metric_series(
            run_collections=mappo_curriculum_social_law_yielding_centralized_runs,
            agent_labels=agents,
            returns_metric="Eval/Return",
            collisions_metric="Eval/Collisions",
            max_workers=args.max_workers,
        )
        print_collection_summary("MAPPO (Curriculum - Social Law Yielding + Centralized)", mappo_curriculum_social_law_yielding_centralized_returns_by_agent, mappo_curriculum_social_law_yielding_centralized_collisions_by_agent)
        ppo_returns_by_agent, ppo_collisions_by_agent = collect_metric_series(
            run_collections=ppo_runs,
            agent_labels=agents,
            returns_metric="Eval/Single_Agent_Proj_Joint/Return",
            collisions_metric="Eval/Single_Agent_Proj_Joint/Collisions",
            ppo_per_agent_metrics=False,
            max_workers=args.max_workers,
        )
        print_collection_summary("PPO", ppo_returns_by_agent, ppo_collisions_by_agent)
        ppo_social_law_returns_by_agent, ppo_social_law_collisions_by_agent = collect_metric_series(
            run_collections=ppo_social_law_runs,
            agent_labels=agents,
            returns_metric="Eval/Single_Agent_Proj_Joint/Return",
            collisions_metric="Eval/Single_Agent_Proj_Joint/Collisions",
            ppo_per_agent_metrics=False,
        ...
        print_collection_summary("PPO (Social Law)", ppo_social_law_returns_by_agent, ppo_social_law_collisions_by_agent)
        ppo_social_law_yielding_returns_by_agent, ppo_social_law_yielding_collisions_by_agent = collect_metric_series(
            run_collections=ppo_social_law_yielding_runs,
            agent_labels=agents,
            returns_metric="Eval/Single_Agent_Proj_Joint/Return",
            collisions_metric="Eval/Single_Agent_Proj_Joint/Collisions",
            ppo_per_agent_metrics=False,
        ...
        print_collection_summary("PPO (Social Law Yielding)", ppo_social_law_yielding_returns_by_agent, ppo_social_law_yielding_collisions_by_agent)

    # Save processed collections to cache to speed up subsequent runs
    if args.data_cache:
        cache_dir = os.path.dirname(args.data_cache)
        if cache_dir:
            os.makedirs(cache_dir, exist_ok=True)
        cached = {
            # "ippo_returns_by_agent": ippo_returns_by_agent,
            # "ippo_collisions_by_agent": ippo_collisions_by_agent,
            "ippo_centralized_returns_by_agent": ippo_centralized_returns_by_agent,
            "ippo_centralized_collisions_by_agent": ippo_centralized_collisions_by_agent,
            ...
            # "ippo_curriculum_social_law_returns_by_agent": ippo_curriculum_social_law_returns_by_agent,
            # "ippo_curriculum_social_law_collisions_by_agent": ippo_curriculum_social_law_collisions_by_agent,
            "ippo_curriculum_social_law_centralized_returns_by_agent": ippo_curriculum_social_law_centralized_returns_by_agent,
            "ippo_curriculum_social_law_centralized_collisions_by_agent": ippo_curriculum_social_law_centralized_collisions_by_agent,
            # "ippo_curriculum_social_law_yielding_returns_by_agent": ippo_curriculum_social_law_yielding_returns_by_agent,
            # "ippo_curriculum_social_law_yielding_collisions_by_agent": ippo_curriculum_social_law_yielding_collisions_by_agent,
            "ippo_curriculum_social_law_yielding_centralized_returns_by_agent": ippo_curriculum_social_law_yielding_centralized_returns_by_agent,
            "ippo_curriculum_social_law_yielding_centralized_collisions_by_agent": ippo_curriculum_social_law_yielding_centralized_collisions_by_agent,
            "mappo_centralized_returns_by_agent": mappo_centralized_returns_by_agent,
            "mappo_centralized_collisions_by_agent": mappo_centralized_collisions_by_agent,
            ...
            "mappo_curriculum_social_law_centralized_returns_by_agent": mappo_curriculum_social_law_centralized_returns_by_agent,
            "mappo_curriculum_social_law_centralized_collisions_by_agent": mappo_curriculum_social_law_centralized_collisions_by_agent,
            "mappo_curriculum_social_law_yielding_centralized_returns_by_agent": mappo_curriculum_social_law_yielding_centralized_returns_by_agent,
            "mappo_curriculum_social_law_yielding_centralized_collisions_by_agent": mappo_curriculum_social_law_yielding_centralized_collisions_by_agent,
            "ppo_returns_by_agent": ppo_returns_by_agent,
            "ppo_collisions_by_agent": ppo_collisions_by_agent,
            "ppo_social_law_returns_by_agent": ppo_social_law_returns_by_agent,
            "ppo_social_law_collisions_by_agent": ppo_social_law_collisions_by_agent,
            "ppo_social_law_yielding_returns_by_agent": ppo_social_law_yielding_returns_by_agent,
            "ppo_social_law_yielding_collisions_by_agent": ppo_social_law_yielding_collisions_by_agent,
        }
    ...
    for agent_label in agents:
        ...
        # grouped_returns[f"IPPO (Curriculum - Social Law) | {agent_label}"] = ippo_curriculum_social_law_returns_by_agent[agent_label]
        grouped_returns[f"IPPO (Curriculum - Social Law + Centralized) | {agent_label}"] = ippo_curriculum_social_law_centralized_returns_by_agent[agent_label]
        # grouped_returns[f"IPPO (Curriculum - Social Law Yielding) | {agent_label}"] = ippo_curriculum_social_law_yielding_returns_by_agent[agent_label]
        grouped_returns[f"IPPO (Curriculum - Social Law Yielding + Centralized) | {agent_label}"] = ippo_curriculum_social_law_yielding_centralized_returns_by_agent[agent_label]
        ...
        grouped_returns[f"MAPPO (Curriculum - Social Law + Centralized) | {agent_label}"] = mappo_curriculum_social_law_centralized_returns_by_agent[agent_label]
        grouped_returns[f"MAPPO (Curriculum - Social Law Yielding + Centralized) | {agent_label}"] = mappo_curriculum_social_law_yielding_centralized_returns_by_agent[agent_label]

        # PPO (no longer per-agent for eval plots)
        grouped_returns[f"PPO | {agent_label}"] = ppo_returns_by_agent[agent_label]
        grouped_returns[f"PPO (Social Law) | {agent_label}"] = ppo_social_law_returns_by_agent[agent_label]
        grouped_returns[f"PPO (Social Law Yielding) | {agent_label}"] = ppo_social_law_yielding_returns_by_agent[agent_label]
        ...
        # grouped_collisions[f"IPPO (Curriculum - Social Law) | {agent_label}"] = ippo_curriculum_social_law_collisions_by_agent[agent_label]
        grouped_collisions[f"IPPO (Curriculum - Social Law + Centralized) | {agent_label}"] = ippo_curriculum_social_law_centralized_collisions_by_agent[agent_label]
        # grouped_collisions[f"IPPO (Curriculum - Social Law Yielding) | {agent_label}"] = ippo_curriculum_social_law_yielding_collisions_by_agent[agent_label]
        grouped_collisions[f"IPPO (Curriculum - Social Law Yielding + Centralized) | {agent_label}"] = ippo_curriculum_social_law_yielding_centralized_collisions_by_agent[agent_label]
        ...
        grouped_collisions[f"MAPPO (Curriculum - Social Law + Centralized) | {agent_label}"] = mappo_curriculum_social_law_centralized_collisions_by_agent[agent_label]
        grouped_collisions[f"MAPPO (Curriculum - Social Law Yielding + Centralized) | {agent_label}"] = mappo_curriculum_social_law_yielding_centralized_collisions_by_agent[agent_label]

        # PPO (no longer per-agent for eval plots)
        grouped_collisions[f"PPO | {agent_label}"] = ppo_collisions_by_agent[agent_label]
        grouped_collisions[f"PPO (Social Law) | {agent_label}"] = ppo_social_law_collisions_by_agent[agent_label]
        grouped_collisions[f"PPO (Social Law Yielding) | {agent_label}"] = ppo_social_law_yielding_collisions_by_agent[agent_label]
