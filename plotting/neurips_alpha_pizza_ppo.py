...
def main() -> None:
    ...
    ppo_generalized_filters = []
    for agent in agents:
        ppo_generalized_filters.append({
            "tags": {
                "$all": [agent, "social_law_generalization", "ppo"],
                "$nin": ["OLD", "social law"],
            }
        })

    ppo_generalized_social_law_filters = []
    for agent in agents:
        ppo_generalized_social_law_filters.append({
            "tags": {
                "$all": [agent, "social_law_generalization", "ppo", "social law"],
                "$nin": ["OLD"],
            }
        })
    ...
    ppo_generalized_runs = []
    for ppo_generalized_filter in ppo_generalized_filters:
        ppo_generalized_runs.append(api.runs(path=path, filters=ppo_generalized_filter, per_page=args.max_runs))

    ppo_generalized_social_law_runs = []
    for ppo_generalized_social_law_filter in ppo_generalized_social_law_filters:
        ppo_generalized_social_law_runs.append(api.runs(path=path, filters=ppo_generalized_social_law_filter, per_page=args.max_runs))


    for i in range(len(agents)):
        print(f"PPO {agents[i]} runs: {len(ppo_runs[i])}")
        print(f"PPO (Social Law) {agents[i]} runs: {len(ppo_social_law_runs[i])}")
        print(f"PPO (Generalized) {agents[i]} runs: {len(ppo_generalized_runs[i])}")
        print(f"PPO (Generalized, Social Law) {agents[i]} runs: {len(ppo_generalized_social_law_runs[i])}")
    ...
    ppo_generalized_returns_by_agent, ppo_generalized_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_runs,
        agent_labels=agents,
        returns_metric="Train/returned_episode_returns",
        collisions_metric="Train/returned_episode_collisions",
        ppo_per_agent_metrics=True,
    ...
    print_collection_summary("PPO (Generalized)", ppo_generalized_returns_by_agent, ppo_generalized_collisions_by_agent)
    ppo_generalized_social_law_returns_by_agent, ppo_generalized_social_law_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_social_law_runs,
        agent_labels=agents,
        returns_metric="Train/returned_episode_returns",
        collisions_metric="Train/returned_episode_collisions",
        ppo_per_agent_metrics=True,
    ...
    print_collection_summary("PPO (Generalized, Social Law)", ppo_generalized_social_law_returns_by_agent, ppo_generalized_social_law_collisions_by_agent)
    # Create separate plots per agent-count group and separate returns/collisions plots.
    # Collect worst-case and best-case metrics for PPO variants
    ppo_worst_case_by_agent, ppo_worst_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_runs,
        agent_labels=agents,
    ...
    ppo_best_case_by_agent, ppo_best_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_runs,
        agent_labels=agents,
    ...
    ppo_sl_worst_case_by_agent, ppo_sl_worst_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_social_law_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_collisions",
    ...
    ppo_sl_best_case_by_agent, ppo_sl_best_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_social_law_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_collisions",
    ...
    ppo_gen_worst_case_by_agent, ppo_gen_worst_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_collisions",
    ...
    ppo_gen_best_case_by_agent, ppo_gen_best_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_collisions",
    ...
    ppo_gen_sl_worst_case_by_agent, ppo_gen_sl_worst_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_social_law_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/WorstCase/returned_episode_collisions",
    ...
    ppo_gen_sl_best_case_by_agent, ppo_gen_sl_best_case_collisions_by_agent = collect_metric_series(
        run_collections=ppo_generalized_social_law_runs,
        agent_labels=agents,
        returns_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_returns",
        collisions_metric="Train/Joint/Agent_1_Optimize/BestCase/returned_episode_collisions",
    ...
    alpha_variant_specs: list[tuple[str, list[Any]]] = [
        ("PPO", ppo_runs),
        ("PPO (Social Law)", ppo_social_law_runs),
        ("PPO (Generalized)", ppo_generalized_runs),
        ("PPO (Generalized, Social Law)", ppo_generalized_social_law_runs),
    ]
    ...
    for agent_label in agents:
        ...
        ppo_gen_groups = ppo_generalized_returns_by_agent[agent_label]
        for ai in range(agent_count):
            series_for_agent = []
            for run_series_list in ppo_gen_groups:
                if isinstance(run_series_list, list) and len(run_series_list) > ai:
                    series_for_agent.append(run_series_list[ai])
            grouped_returns[f"PPO (Generalized) | {agent_label} | Agent {ai+1}"] = series_for_agent

        ppo_gen_sl_groups = ppo_generalized_social_law_returns_by_agent[agent_label]
        for ai in range(agent_count):
            series_for_agent = []
            for run_series_list in ppo_gen_sl_groups:
                if isinstance(run_series_list, list) and len(run_series_list) > ai:
                    series_for_agent.append(run_series_list[ai])
            grouped_returns[f"PPO (Generalized, Social Law) | {agent_label} | Agent {ai+1}"] = series_for_agent
        ...
        ppo_gen_groups_c = ppo_generalized_collisions_by_agent[agent_label]
        for ai in range(agent_count):
            series_for_agent = []
            for run_series_list in ppo_gen_groups_c:
                if isinstance(run_series_list, list) and len(run_series_list) > ai:
                    series_for_agent.append(run_series_list[ai])
            grouped_collisions[f"PPO (Generalized) | {agent_label} | Agent {ai+1}"] = series_for_agent

        ppo_gen_sl_groups_c = ppo_generalized_social_law_collisions_by_agent[agent_label]
        for ai in range(agent_count):
            series_for_agent = []
            for run_series_list in ppo_gen_sl_groups_c:
                if isinstance(run_series_list, list) and len(run_series_list) > ai:
                    series_for_agent.append(run_series_list[ai])
            grouped_collisions[f"PPO (Generalized, Social Law) | {agent_label} | Agent {ai+1}"] = series_for_agent

        # Build grouped worst-case series for this agent_label only.
        grouped_worst_case: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": ppo_worst_case_by_agent[agent_label],
            f"PPO (Social Law) | {agent_label}": ppo_sl_worst_case_by_agent[agent_label],
            f"PPO (Generalized) | {agent_label}": ppo_gen_worst_case_by_agent[agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": ppo_gen_sl_worst_case_by_agent[agent_label],
        }

        grouped_worst_case_collisions: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": ppo_worst_case_collisions_by_agent[agent_label],
            f"PPO (Social Law) | {agent_label}": ppo_sl_worst_case_collisions_by_agent[agent_label],
            f"PPO (Generalized) | {agent_label}": ppo_gen_worst_case_collisions_by_agent[agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": ppo_gen_sl_worst_case_collisions_by_agent[agent_label],
        }

        # Build grouped best-case series for this agent_label only.
        grouped_best_case: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": ppo_best_case_by_agent[agent_label],
            f"PPO (Social Law) | {agent_label}": ppo_sl_best_case_by_agent[agent_label],
            f"PPO (Generalized) | {agent_label}": ppo_gen_best_case_by_agent[agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": ppo_gen_sl_best_case_by_agent[agent_label],
        }

        grouped_best_case_collisions: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": ppo_best_case_collisions_by_agent[agent_label],
            f"PPO (Social Law) | {agent_label}": ppo_sl_best_case_collisions_by_agent[agent_label],
            f"PPO (Generalized) | {agent_label}": ppo_gen_best_case_collisions_by_agent[agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": ppo_gen_sl_best_case_collisions_by_agent[agent_label],
        }
        ...
        grouped_eval_worst: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": alpha_eval_worst_by_variant["PPO"][agent_label],
            f"PPO (Social Law) | {agent_label}": alpha_eval_worst_by_variant["PPO (Social Law)"][agent_label],
            f"PPO (Generalized) | {agent_label}": alpha_eval_worst_by_variant["PPO (Generalized)"][agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": alpha_eval_worst_by_variant["PPO (Generalized, Social Law)"][agent_label],
        }

        grouped_eval_optimal: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": alpha_eval_optimal_by_variant["PPO"][agent_label],
            f"PPO (Social Law) | {agent_label}": alpha_eval_optimal_by_variant["PPO (Social Law)"][agent_label],
            f"PPO (Generalized) | {agent_label}": alpha_eval_optimal_by_variant["PPO (Generalized)"][agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": alpha_eval_optimal_by_variant["PPO (Generalized, Social Law)"][agent_label],
        }

        grouped_eval_best: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": alpha_eval_best_by_variant["PPO"][agent_label],
            f"PPO (Social Law) | {agent_label}": alpha_eval_best_by_variant["PPO (Social Law)"][agent_label],
            f"PPO (Generalized) | {agent_label}": alpha_eval_best_by_variant["PPO (Generalized)"][agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": alpha_eval_best_by_variant["PPO (Generalized, Social Law)"][agent_label],
        }

        grouped_eval_single_return: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": alpha_eval_single_return_by_variant["PPO"][agent_label],
            f"PPO (Social Law) | {agent_label}": alpha_eval_single_return_by_variant["PPO (Social Law)"][agent_label],
            f"PPO (Generalized) | {agent_label}": alpha_eval_single_return_by_variant["PPO (Generalized)"][agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": alpha_eval_single_return_by_variant["PPO (Generalized, Social Law)"][agent_label],
        }

        grouped_eval_single_collisions: dict[str, list[RunSeries]] = {
            f"PPO | {agent_label}": alpha_eval_single_collisions_by_variant["PPO"][agent_label],
            f"PPO (Social Law) | {agent_label}": alpha_eval_single_collisions_by_variant["PPO (Social Law)"][agent_label],
            f"PPO (Generalized) | {agent_label}": alpha_eval_single_collisions_by_variant["PPO (Generalized)"][agent_label],
            f"PPO (Generalized, Social Law) | {agent_label}": alpha_eval_single_collisions_by_variant["PPO (Generalized, Social Law)"][agent_label],
        }
