...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # Tag helpers — confirmed from WandB:
    #   No Law:     CoopRecon_Compare_No_law_{N}Agent
    #   Law 0.0:    CoopRecon_Compare_Law_0.0_{N}Agent
    #   Law 0.1:    CoopRecon_Compare_Law_0.1_{N}Agent
    #   Law 0.2:    CoopRecon_Compare_Law_{N}Agent
    #   Curriculum: CoopRecon_Compare_Curriculum_law_0_{x}_{N}Agent
    # ------------------------------------------------------------------
    def _n(lbl):
        return lbl.split()[0]

    def _group_tag(lbl, cond):
        n = _n(lbl)
        tags = {
            "no_law":  f"CoopRecon_Compare_No_law_{n}Agent",
            "law_0.0": f"CoopRecon_Compare_Law_0.0_{n}Agent",
            "law_0.1": f"CoopRecon_Compare_Law_0.1_{n}Agent",
            "law_0.2": f"CoopRecon_Compare_Law_{n}Agent",
            "cur_0.0": f"CoopRecon_Compare_Curriculum_law_0_0_{n}Agent",
            "cur_0.1": f"CoopRecon_Compare_Curriculum_law_0_1_{n}Agent",
            "cur_0.2": f"CoopRecon_Compare_Curriculum_law_0_2_{n}Agent",
        }
        return tags[cond]

    def _mf(algo, cond, exc=None):
        filters = []
        for a in agents:
            f = {"tags": {"$all": [a, algo, _group_tag(a, cond)]}}
            if exc:
                f["tags"]["$nin"] = exc
            filters.append(f)
        return filters

    # IPPO — No Law and Law variants (eval baseline comparison)
    ippo_centralized_filters                          = _mf("ippo",  "no_law",  exc=["curriculum"])
    ippo_social_law_centralized_filters               = _mf("ippo",  "law_0.1", exc=["curriculum"])
    ippo_social_law_yielding_centralized_filters      = _mf("ippo",  "law_0.2", exc=["curriculum"])
    ippo_curriculum_social_law_centralized_filters          = _mf("ippo",  "cur_0.1")
    ippo_curriculum_social_law_yielding_centralized_filters = _mf("ippo",  "cur_0.2")

    # MAPPO
    mappo_centralized_filters                               = _mf("mappo", "no_law",  exc=["curriculum"])
    mappo_social_law_centralized_filters                    = _mf("mappo", "law_0.1", exc=["curriculum"])
    mappo_social_law_yielding_centralized_filters           = _mf("mappo", "law_0.2", exc=["curriculum"])
    mappo_curriculum_social_law_centralized_filters         = _mf("mappo", "cur_0.1")
    mappo_curriculum_social_law_yielding_centralized_filters = _mf("mappo", "cur_0.2")

    # PPO
    ppo_filters                   = _mf("ppo", "no_law",  exc=["creppo"])
    ppo_social_law_filters        = _mf("ppo", "law_0.1", exc=["creppo"])
    ppo_social_law_yielding_filters = _mf("ppo", "law_0.2", exc=["creppo"])

    def _runs(filters):
        return [api.runs(path=path, filters=f, per_page=args.max_runs) for f in filters]

    ippo_centralized_runs                          = _runs(ippo_centralized_filters)
    ippo_social_law_centralized_runs               = _runs(ippo_social_law_centralized_filters)
    ippo_social_law_yielding_centralized_runs      = _runs(ippo_social_law_yielding_centralized_filters)
    ippo_curriculum_social_law_centralized_runs          = _runs(ippo_curriculum_social_law_centralized_filters)
    ippo_curriculum_social_law_yielding_centralized_runs = _runs(ippo_curriculum_social_law_yielding_centralized_filters)

    mappo_centralized_runs                               = _runs(mappo_centralized_filters)
    mappo_social_law_centralized_runs                    = _runs(mappo_social_law_centralized_filters)
    mappo_social_law_yielding_centralized_runs           = _runs(mappo_social_law_yielding_centralized_filters)
    mappo_curriculum_social_law_centralized_runs         = _runs(mappo_curriculum_social_law_centralized_filters)
    mappo_curriculum_social_law_yielding_centralized_runs = _runs(mappo_curriculum_social_law_yielding_centralized_filters)

    ppo_runs                   = _runs(ppo_filters)
    ppo_social_law_runs        = _runs(ppo_social_law_filters)
    ppo_social_law_yielding_runs = _runs(ppo_social_law_yielding_filters)

    for i in range(len(agents)):
        print(f"IPPO (No Law) {agents[i]} runs: {len(ippo_centralized_runs[i])}")
        print(f"IPPO (Law 0.1) {agents[i]} runs: {len(ippo_social_law_centralized_runs[i])}")
        print(f"IPPO (Law 0.2) {agents[i]} runs: {len(ippo_social_law_yielding_centralized_runs[i])}")
        print(f"IPPO Curriculum 0.1 {agents[i]} runs: {len(ippo_curriculum_social_law_centralized_runs[i])}")
        print(f"IPPO Curriculum 0.2 {agents[i]} runs: {len(ippo_curriculum_social_law_yielding_centralized_runs[i])}")
        print(f"MAPPO (No Law) {agents[i]} runs: {len(mappo_centralized_runs[i])}")
        print(f"MAPPO (Law 0.1) {agents[i]} runs: {len(mappo_social_law_centralized_runs[i])}")
        print(f"MAPPO (Law 0.2) {agents[i]} runs: {len(mappo_social_law_yielding_centralized_runs[i])}")
        print(f"MAPPO Curriculum 0.1 {agents[i]} runs: {len(mappo_curriculum_social_law_centralized_runs[i])}")
        print(f"MAPPO Curriculum 0.2 {agents[i]} runs: {len(mappo_curriculum_social_law_yielding_centralized_runs[i])}")
        print(f"PPO (No Law) {agents[i]} runs: {len(ppo_runs[i])}")
        print(f"PPO (Law 0.1) {agents[i]} runs: {len(ppo_social_law_runs[i])}")
        print(f"PPO (Law 0.2) {agents[i]} runs: {len(ppo_social_law_yielding_runs[i])}")
