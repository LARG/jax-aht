'''
HDR-IPPO ego training: PPO ego agent with a frozen BC-LSTM anchor that regularizes
the ego policy via forward KL divergence KL(pi_BC || pi_ego).

Supports two training modes controlled by the SELF_PLAY config flag:
  - SELF_PLAY=true (default): both agents share ego_policy with train_state.params.
  - SELF_PLAY=false: agent_1 is drawn from a frozen partner_population (same as ppo_ego.py).

The core HDR change is in _loss_fn: after computing the standard PPO loss, we run
a frozen BC-LSTM forward pass over the trajectory to get bc_pi, then add
    BC_POLICY_KL_WEIGHT * KL(bc_pi || ego_pi)
to the total loss. This encourages mode-covering behavior relative to the anchor.

Command to run HDR-IPPO ego training:
python ego_agent_training/run.py algorithm=hdr_ippo_ego/<task> task=<task> label=test_hdr_ippo

Suggested debug command:
python ego_agent_training/run.py algorithm=hdr_ippo_ego/<task> task=<task> logger.mode=disabled label=debug algorithm.TOTAL_TIMESTEPS=1e5
'''
import os
import shutil
import time
...
def _load_bc_anchor(config):
    '''Load the frozen BC-LSTM anchor network and parameters from config paths.

    Returns:
        bc_params: frozen parameter dict (never updated during training)
        bc_network: BCLSTMNetwork instance
        bc_lstm_dim: int, LSTM hidden dimension
    '''
    bc_config_path = config["BC_ANCHOR_CONFIG_PATH"]
    if not os.path.isabs(bc_config_path):
        bc_config_path = os.path.join(REPO_PATH, bc_config_path)
    with open(bc_config_path) as f:
        bc_cfg = yaml.safe_load(f)

    bc_lstm_config = BCLSTMConfig(
        obs_dim=bc_cfg['obs_dim'],
        action_dim=bc_cfg['action_dim'],
        lstm_dim=bc_cfg.get('lstm_dim', 512),
        preprocess_dims=tuple(bc_cfg.get('preprocess_dims', [1024])),
        postprocess_dims=tuple(bc_cfg.get('postprocess_dims', [256])),
    )
    bc_anchor_path = config["BC_ANCHOR_PATH"]
    if not os.path.isabs(bc_anchor_path):
        bc_anchor_path = os.path.join(REPO_PATH, bc_anchor_path)
    bc_agent = BCLSTMAgent(bc_lstm_config, weight_path=bc_anchor_path)
    bc_params = bc_agent.params  # Frozen, never updated
    bc_network = bc_agent.network
    bc_lstm_dim = bc_lstm_config.lstm_dim
    return bc_params, bc_network, bc_lstm_dim


def train_hdr_ippo_ego_agent(config, env, train_rng,
                              ego_policy, init_ego_params, n_ego_train_seeds,
                              partner_population, partner_params,
                              bc_params, bc_network, bc_lstm_dim):
    '''
    Train HDR-IPPO ego agent. In self-play mode, both agents share ego_policy.
    Otherwise, agent_1 is drawn from a frozen partner_population.
    ...
    Args:
        ...
        partner_population: AgentPopulation or None, population of partner agents (None when self-play)
        partner_params: pytree of partner parameters or None (None when self-play)
        bc_params: frozen BC-LSTM parameter dict
        bc_network: BCLSTMNetwork instance
        bc_lstm_dim: int, LSTM hidden dimension
    '''
    self_play = config.get("SELF_PLAY", True)

    num_total_partners = partner_population.pop_size if partner_population is not None else 0
    ...
    def make_ppo_train(config):
        ...
        def train(rng):
            ...
            if self_play:
                init_agent1_hstate = ego_policy.init_hstate(config["NUM_CONTROLLED_ACTORS"])
            else:
                init_partner_hstate = partner_population.init_hstate(config["NUM_UNCONTROLLED_ACTORS"])

            def _env_step_self_play(runner_state, unused):
                train_state, env_state, prev_obs, prev_done, ego_hstate, agent1_hstate, rng = runner_state
                rng, actor_rng, agent1_rng, step_rng = jax.random.split(rng, 4)
                ...
                act_1, new_agent1_hstate = ego_policy.get_action(
                    params=train_state.params,
                    obs=prev_obs["agent_1"].reshape(1, config["NUM_CONTROLLED_ACTORS"], -1),
                    done=prev_done["agent_1"].reshape(1, config["NUM_CONTROLLED_ACTORS"]),
                    avail_actions=avail_actions_1,
                    hstate=agent1_hstate,
                    rng=agent1_rng
                )
                act_1 = act_1.squeeze()
                ...
                new_runner_state = (train_state, env_state_next, obs_next, done_next,
                                    new_ego_hstate, new_agent1_hstate, rng)
                return new_runner_state, transition

            def _env_step_population(runner_state, unused):
            ...
            def _update_minbatch(train_state, batch_info):
                ...
                def _loss_fn(params, init_ego_hstate, traj_batch, gae, target_v):
                    ...
                    mb_size = traj_batch.obs.shape[1]
                    bc_carry_init = (jnp.zeros((mb_size, bc_lstm_dim)),
                                     jnp.zeros((mb_size, bc_lstm_dim)))

                    def bc_step(carry, step_data):
                        obs_t, done_t = step_data
                        carry = jax.tree.map(
                            lambda c: jnp.where(done_t[:, None], jnp.zeros_like(c), c),
                            carry
                        )
                        carry, logits = bc_network.apply({'params': bc_params}, carry, obs_t)
                        return carry, logits

                    _, bc_logits = jax.lax.scan(
                        bc_step, bc_carry_init, (traj_batch.obs, traj_batch.done)
                    )
                    bc_logits = jnp.where(traj_batch.avail_actions > 0, bc_logits, -1e9)
                    bc_pi = distrax.Categorical(logits=jax.lax.stop_gradient(bc_logits))

                    kl_loss = bc_pi.kl_divergence(pi).mean()

                    ippo_loss = pg_loss + config["VF_COEF"] * value_loss - config["ENT_COEF"] * entropy
                    total_loss = (1.0 - config["BC_POLICY_KL_WEIGHT"]) * ippo_loss + config["BC_POLICY_KL_WEIGHT"] * kl_loss
                    return total_loss, (value_loss, pg_loss, entropy, kl_loss)
            ...
            def _update_step_self_play(update_runner_state, unused):
                ...
                runner_state = (train_state, init_env_state, init_obs, init_done,
                                init_ego_hstate, init_agent1_hstate, rng)

                runner_state, traj_batch = jax.lax.scan(
                    _env_step_self_play, runner_state, None, config["ROLLOUT_LENGTH"])
                (train_state, env_state, obs, done, ego_hstate, agent1_hstate, rng) = runner_state
            ...
            def _update_step_population(update_runner_state, unused):
                ...
                runner_state, traj_batch = jax.lax.scan(
                    _env_step_population, runner_state, None, config["ROLLOUT_LENGTH"])
            ...
            _update_step = _update_step_self_play if self_play else _update_step_population
                ...
                def _update_step_with_ckpt(state_with_ckpt, unused):
                    ...
                    def store_and_eval_ckpt(args):
                        ...
                        eval_eps_last_infos = run_episodes(
                            ...
                            agent_1_param=train_state.params, agent_1_policy=ego_policy,
                            max_episode_steps=max_episode_steps,
                            num_eps=config["NUM_EVAL_EPISODES"])
                        eval_eps_last_infos = jax.tree.map(lambda x: x[None, ...], eval_eps_last_infos)
                    ...
                    for key, val in metric.items():
                        if key in ("update_steps", "actor_loss", "value_loss",
                                   "entropy_loss", "kl_loss", "avg_grad_norm"):
...
def run_ego_training(config, wandb_logger):
    '''Run HDR-IPPO ego agent training.

    In SELF_PLAY mode, the ego trains against itself (no partner population needed).
    Otherwise, trains against a frozen partner population (same as ppo_ego.py).

    Args:
        config: dict, config for the training
        wandb_logger: logger instance
    '''
    algorithm_config = dict(config["algorithm"])
    self_play = algorithm_config.get("SELF_PLAY", True)
    ...
    bc_params, bc_network, bc_lstm_dim = _load_bc_anchor(algorithm_config)
    log.info("Loaded BC-LSTM anchor (lstm_dim=%d)", bc_lstm_dim)

    partner_population = None
    flattened_partner_params = None
    pop_size = 0
    ...
    mode_str = "self-play" if self_play else f"population ({pop_size} partner(s))"
    log.info(
        "Starting HDR-IPPO ego training [%s] with %s updates, %s envs, BC_POLICY_KL_WEIGHT=%.4f. "
        "Progress will be logged every ~%s updates.",
        mode_str,
        expected_updates,
        algorithm_config["NUM_ENVS"],
        algorithm_config["BC_POLICY_KL_WEIGHT"],
        progress_interval,
    )
    start_time = time.time()

    out = train_hdr_ippo_ego_agent(
...
def log_metrics(config, train_out, logger, metric_names: tuple):
    ...
    Args:
        ...
        train_out: dict, the output from training
    ...
    all_ego_kl_losses = np.asarray(train_metrics["kl_loss"])
    ...
    average_ego_kl_losses = np.mean(all_ego_kl_losses, axis=0)
    ...
    for step in range(num_updates):
        ...
        logger.log_item("Train/EgoKLLoss", average_ego_kl_losses[step], train_step=step, commit=True)
