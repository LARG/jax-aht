'''
Script for training a LILI ego agent against a population of homogeneous partner agents.

LILI extends LIAM by incorporating language-like instruction generation and influence mechanisms
to better coordinate with partner agents through learned instruction representations.
...
Command to run LILI ego training:

python ego_agent_training/run.py algorithm=lili_ego/lbf task=lbf label=test_lili_ego

Suggested debug command:

python ego_agent_training/run.py algorithm=lili_ego/lbf task=lbf logger.mode=disabled label=debug algorithm.TOTAL_TIMESTEPS=1e5
'''
import shutil
import time
import logging
...
from agents.initialize_agents import initialize_lili_agent
...
from marl.lili_utils import Transition

log = logging.getLogger(__name__)
logging.basicConfig(level=logging.INFO)


def train_lili_ego_agent(config, env, train_rng,
                        ego_policy, init_ego_params, n_ego_train_seeds,
                        partner_population: AgentPopulation,
                        partner_params
                        ):
    '''
    Train LILI ego agent using the given partner checkpoints and initial ego parameters.
    ...
    # ------------------------------
    # Build the LILI training function
    # ------------------------------
    def make_lili_train(config):
        ...
        def train(rng):
            ...
            def _env_step(runner_state, unused):
                ...
                """
                train_state, encoder_decoder_train_state, env_state, prev_obs, prev_done, act_onehot, ego_hstate, partner_hstate, partner_indices, rng, episode_ids = runner_state
                ...
                episode_ids = jax.vmap(lambda id, done: id + 1 if done else id, in_axes=0)(episode_ids, done_next)
                ...
                transition = Transition(
                    ...
                    episode_ids=jnp.zeros_like(done_next["agent_0"])  
                )
                new_runner_state = (train_state, encoder_decoder_train_state, env_state_next, obs_next, done_next, env_act_onehot,
                                    new_ego_hstate, new_partner_hstate, updated_partner_indices, rng, episode_ids)
            ...
            # traj_batch.obs: dict of agent -> (rollout_len, num_controlled_actors, obs_dim)
            # traj_batch.episode_ids: (rollout_len, num_controlled_actors)
            def stack_k_obs(obs_arr, episode_ids_arr, K=4):
                # obs_arr: (rollout_len, num_controlled_actors, obs_dim)
                # episode_ids_arr: (rollout_len, num_controlled_actors)
                print(obs_arr.shape)
                rollout_len, num_controlled_actors, obs_dim = obs_arr.shape
                stacked_obs = []
                for t in range(rollout_len):
                    ...
                    for k in range(K):
                        ...
                        if idx < 0:
                            ...
                            stacked.append(jnp.zeros((num_controlled_actors, obs_dim), dtype=obs_arr.dtype))
                        else:
                            # Only use obs if episode_ids match (i.e., not a new episode)
                            match = (episode_ids_arr[t, :] == episode_ids_arr[idx, :])
                            stacked_obs_k = jnp.where(
                                match[:, None],
                                obs_arr[idx],
                                jnp.zeros((num_controlled_actors, obs_dim), dtype=obs_arr.dtype)
                            )
                            stacked.append(stacked_obs_k)
                    # Stack along new axis (K, num_controlled_actors, obs_dim) -> (num_controlled_actors, K*obs_dim)
                    stacked = jnp.concatenate(stacked[::-1], axis=-1)
                    stacked_obs.append(stacked)
                # (rollout_len, num_controlled_actors, K*obs_dim)
                return jnp.stack(stacked_obs, axis=0)           
            ...
            def _update_step(update_runner_state, unused):
                ...
                episode_ids = jnp.zeros((config["NUM_ENVS"]), dtype=jnp.int32)

                # 1) rollout
                runner_state = (train_state, encoder_decoder_train_state, init_env_state, init_obs, init_done, init_act_onehot, init_ego_hstate, init_partner_hstate, new_partner_indices, rng, episode_ids)
                runner_state, traj_batch = jax.lax.scan(
                    _env_step, runner_state, None, config["ROLLOUT_LENGTH"])
                (train_state, encoder_decoder_train_state, env_state, obs, done, act_onehot, ego_hstate, partner_hstate, partner_indices, rng, episode_ids) = runner_state

                print(traj_batch.obs.shape)
                print(config["ROLLOUT_LENGTH"])
                print(config["NUM_ENVS"])

                obs = obs["agent_0"].reshape(1, config["NUM_CONTROLLED_ACTORS"], -1)
                traj_batch = traj_batch._replace(obs=stack_k_obs(traj_batch.obs, traj_batch.episode_ids)) 
                #act_onehot = {k: stack_k_obs(traj_batch.act_onehot[k], traj_batch.episode_ids) for k in traj_batch.act_onehot}
                ...
                metric["partner_modeling_loss"] = loss_terms[3]
                # LILI-specific metrics
                metric["influence_loss"] = loss_terms[4]
                metric["coordination_loss"] = loss_terms[5]
                metric["behavioral_shaping_loss"] = loss_terms[6]
            ...
            # LILI Update and Checkpoint saving
    ...
    # ------------------------------
    # Actually run the LILI training
    # ------------------------------
    rngs = jax.random.split(train_rng, n_ego_train_seeds)
    train_fn = jax.jit(jax.vmap(make_lili_train(config)))
    out = train_fn(rngs)
    return out
...
def run_ego_training(config, wandb_logger):
    ...
    ego_policy, init_ego_params = initialize_lili_agent(algorithm_config, env, init_ego_rng)

    log.info("Starting ego agent training...")
    start_time = time.time()

    # Run the training
    out = train_lili_ego_agent(
...
def log_metrics(config, train_out, logger, metric_names: tuple):
    ...
    all_ego_partner_modeling_losses = np.asarray(train_metrics["partner_modeling_loss"]) # shape (n_ego_train_seeds, num_updates, num_partners, num_minibatches)
    all_ego_influence_losses = np.asarray(train_metrics["influence_loss"]) # shape (n_ego_train_seeds, num_updates, num_partners, num_minibatches)
    all_ego_coordination_losses = np.asarray(train_metrics["coordination_loss"]) # shape (n_ego_train_seeds, num_updates, num_partners, num_minibatches)
    all_ego_behavioral_shaping_losses = np.asarray(train_metrics["behavioral_shaping_loss"]) # shape (n_ego_train_seeds, num_updates, num_partners, num_minibatches)
    ...
    average_ego_partner_modeling_losses = np.mean(all_ego_partner_modeling_losses, axis=(0, 2, 3))
    average_ego_influence_losses = np.mean(all_ego_influence_losses, axis=(0, 2, 3))
    average_ego_coordination_losses = np.mean(all_ego_coordination_losses, axis=(0, 2, 3))
    average_ego_behavioral_shaping_losses = np.mean(all_ego_behavioral_shaping_losses, axis=(0, 2, 3))
    ...
    for step in range(num_updates):
        ...
        logger.log_item("Train/EgoPartnerModelingLoss", average_ego_partner_modeling_losses[step], train_step=step, commit=True)
        logger.log_item("Train/EgoInfluenceLoss", average_ego_influence_losses[step], train_step=step, commit=True)
        logger.log_item("Train/EgoCoordinationLoss", average_ego_coordination_losses[step], train_step=step, commit=True)
        logger.log_item("Train/EgoBehavioralShapingLoss", average_ego_behavioral_shaping_losses[step], train_step=step, commit=True)
