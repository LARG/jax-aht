...
from common.liam_utils import Transition, unbatchify
...
class DecoderNetwork(nn.Module):
    ...
    def __call__(self, x):
        ...
        prob1 = nn.softmax(prob1)
        prob2 = nn.Dense(self.ouput_dim2)(h2)
        prob2 = nn.softmax(prob2)
        prob3 = nn.Dense(self.ouput_dim2)(h2)
        prob3 = nn.softmax(prob3)
...
def _create_minibatches(traj_batch, advantages, targets, init_hstate, num_actors, num_minibatches, perm_rng):
    """Create minibatches for LIAM updates, where each leaf has shape 
...
def train_liam_ego_agent(config, env, train_rng, 
    ...
    def make_liam_train(config):
        ...
        def train(rng):
            ...
            # 3e) LIAM Update and Checkpoint saving
