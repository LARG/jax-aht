...
def _log_lbrdiv_intermediate_metrics(logger, metric, seed_idx):
    ...
    loss_scalars = {
        ...
        "AvgLMsHorizontal": float(np.asarray(metric["lms_horizontal"], dtype=np.float32).mean()),
        "AvgLMsVertical": float(np.asarray(metric["lms_vertical"], dtype=np.float32).mean()),
    }
...
def train_lbrdiv_partners(train_rng, seed_idx, env, config, conf_policy, br_policy, wandb_logger):
        ...
        def train(rng):
            ...
            def _update_step_with_ckpt(state_with_ckpt, unused):
                ...
                if wandb_logger is not None:
                    io_callback(
                        lambda callback_metric, callback_seed_idx: _log_lbrdiv_intermediate_metrics(
