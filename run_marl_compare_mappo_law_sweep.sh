#!/bin/bash
# Phase E: MARL Comparison MAPPO — Social Law Sweep (aaronson/mckennie)
...
# Ensure venv is active - Prioritize 'venv' as it works on debruyne
...
echo "Starting MAPPO Social Law Sweep Comparisons (0.0 and 0.1 only)..."
for SEED in 72128 721280 721281 721282 721283; do
    echo "=== Running MAPPO with SEED=$SEED ==="

    echo "--- Variation: Law 0.0 ---"
    run_cond "coop_recon_compare_law_0.0" "mappo_law_0.0" $SEED
    echo "--- Finished Law 0.0 for seed $SEED ---"

    echo "--- Variation: Law 0.1 ---"
    run_cond "coop_recon_compare_law_0.1" "mappo_law_0.1" $SEED
    echo "--- Finished Law 0.1 for seed $SEED ---"

done

echo "All MAPPO social law sweep comparisons finished."
