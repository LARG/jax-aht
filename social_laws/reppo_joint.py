    ...
    def make_reppo_joint_train(config):
        ...
        def train(rng, agent_idx, *optimal_params):
            ...
            for i in range(num_agents):
                q_ts = CustomTrainState.create(
                    apply_fn=joint_policies[i].q_network.apply,
                    params=init_joint_params[i]['q_network']["params"],
                    batch_stats=init_joint_params[i]['q_network']["batch_stats"],
                    tx=tx,
                )
                actor_ts = CustomTrainState.create(
                    apply_fn=joint_policies[i].actor.apply,
                    params=init_joint_params[i]['actor']["params"],
                    batch_stats=init_joint_params[i]['actor']["batch_stats"],
                    tx=tx,
                )
                target_actor_ts = CustomTrainState.create(
                    apply_fn=joint_policies[i].actor.apply,
                    params=deepcopy(init_joint_params[i]['actor']["params"]),
                    batch_stats=deepcopy(init_joint_params[i]['actor']["batch_stats"]),
                    tx=optax.set_to_zero(),
                )
                train_states.append(ReppoTrainState(
                    timesteps=0, n_updates=0, grad_steps=0,
                    actor_train_state=actor_ts,
                    q_network_train_state=q_ts,
                    target_actor_train_state=target_actor_ts,
                ))
            ...
            def _env_step(runner_state, unused):
                ...
                critic_rngs = actor_rngs[num_agents:]
                ...
                new_optimal_critic_hstates = []
                for i in range(num_agents):
                    opt_restricted_i, new_opt_critic_hstate_i = _get_optimal_restricted_avail_actions(
                        obs=prev_obs_per_agent[i],
                        done=prev_done_per_agent[i],
                        avail_actions=avail_actions_per_agent[i],
                        hstate=optimal_hstates[1], # use critic hstate for restrictions
                        optimal_params=optimal_params[i],
                        optimal_policy=optimal_policies[i]
                    )
                    optimal_restricted_avail_actions.append(opt_restricted_i)
                    new_optimal_critic_hstates.append(new_opt_critic_hstate_i)
                ...
                new_actor_hstates = []
                for i in range(num_agents):
                    act_i, iw_i, pi_i, new_actor_hstate_i = joint_policies[i].get_action_importance_policy(
                        params=(train_states[i].actor_train_state.params, train_states[i].actor_train_state.batch_stats),
                        ...
                        hstate=hstates[i][0], # use actor hstate for action sampling
                        rng=actor_rngs[i],
                        temp_schedule=temp_scheduler(train_states[i].n_updates)
                    )
                    logp_i = pi_i.log_prob(act_i)
                    acts.append(act_i.squeeze(axis=0))
                    logps.append(logp_i.squeeze(axis=0))
                    importance_weights.append(iw_i.squeeze(axis=0))
                    new_actor_hstates.append(new_actor_hstate_i)
                ...
                for i in range(num_agents):
                    next_opt_restricted_i, _ = _get_optimal_restricted_avail_actions(
                        ...
                        hstate=new_optimal_critic_hstates[i],
                        optimal_params=optimal_params[i],
                        optimal_policy=optimal_policies[i]
                    )
                    next_optimal_restricted_avail_actions.append(next_opt_restricted_i)

                    next_pi_i = train_states[i].target_actor_train_state.apply_fn(
                        {
                            "params": train_states[i].target_actor_train_state.params,
                            "batch_stats": train_states[i].target_actor_train_state.batch_stats,
                        },
                        (obs_full_next_per_agent[i] if config["JOINT_USE_FULL_OBS"] else obs_next_per_agent[i],
                         next_opt_restricted_i),
                        train=False,
                    )
                    # TODO: Deal with recurrent target

                    _, _, q_vals_i, new_critic_hs_i = joint_policies[i].get_critic_logits_probs_values(
                        ...
                        hstate=hstates[i][1], # use critic hstate for value estimation
                        rng=critic_rngs[i],
                    )
                    next_values.append(jnp.sum(next_pi_i.probs * q_vals_i, axis=-1).squeeze(axis=0))
                    new_critic_hstates.append(new_critic_hs_i)
                    ...
                    transitions.append(
                        Transition(
                            ...
                            next_avail_actions=next_optimal_restricted_avail_actions[i],
                            next_val=next_values[i],
                            importance_weight=importance_weights[i],
                            info=info
                        )
                    )
                    new_hstates.append((new_actor_hstates[i], new_critic_hstates[i], hstates[i][2]))
                    new_optimal_hstates.append((optimal_hstates[i][0], new_optimal_critic_hstates[i], optimal_hstates[i][2]))
            ...
            def _update_step(update_runner_state, unused):
                ...
                for i in range(num_agents):
                    next_pi_i = train_states[i].target_actor_train_state.apply_fn(
                        ...
                        (traj_batches[i].next_obs[-1].reshape(1, config["NUM_CONTROLLED_ACTORS"], -1),
                         traj_batches[i].next_avail_actions[-1]),
                        train=False,
                    )
                    # TODO: Deal with recurrent target

                    rng, sample_rng = jax.random.split(rng)
                    _, last_action_logp_i = next_pi_i.sample_and_log_prob(seed=sample_rng)
                    last_action_logp_i = last_action_logp_i.squeeze(axis=0)
                    next_logp_i = jnp.concatenate(
                        [traj_batches[i].action_logp[1:], last_action_logp_i[None]], axis=0
                    )
                    soft_reward_i = traj_batches[i].reward - config["GAMMA"] * next_logp_i * jnp.exp(
                        train_states[i].actor_train_state.params["log_alpha"]
                    )
                    traj_batches[i] = traj_batches[i].replace(soft_reward=soft_reward_i)
                    _, target_vals_i = jax.lax.scan(
                        compute_nstep_lambda,
                        (
                            traj_batches[i].next_val[-1],
                            jnp.ones_like(traj_batches[i].done[0]),
                            jnp.zeros_like(traj_batches[i].importance_weight[0]),
                        ),
                        traj_batches[i],
                        reverse=True,
                    )
                    target_values.append(target_vals_i)

                # 3) REPPO update for each agent separately using factory
                    ...
                    def _learn_epoch(carry, _):
                        ...
                        def _learn_phase(carry, minibatch_and_target):
                            ...
                            def _actor_loss_fn(params, train_state):
                                ...
                                critic_output = joint_policies[agent_i].q_network.apply(
                                    {
                                ...
                                metrics = {
                                    f"agent_{agent_i}/actor_loss": loss,
                                    f"agent_{agent_i}/target_entropy_loss": jnp.mean(alpha_loss),
                                    f"agent_{agent_i}/lagrangian_loss": jnp.mean(lagrangian_loss),
                                    f"agent_{agent_i}/alpha": alpha,
                                    f"agent_{agent_i}/lagrangian": lagrangian,
                ...
                for i in range(num_agents):
                    ...
                    target_actor_ts = train_states[i].target_actor_train_state.replace(
                        params=deepcopy(train_states[i].actor_train_state.params),
                        batch_stats=deepcopy(train_states[i].actor_train_state.batch_stats),
                    )
            ...
            def _update_step_with_ckpt(state_with_ckpt, unused):
                ...
                def store_and_eval_ckpt(args):
                    ...
                    for i in range(num_agents):
                        ckpt_params_i = {
                            "actor": {
                                "params": train_states[i].actor_train_state.params,
                                "batch_stats": train_states[i].actor_train_state.batch_stats
                            },
                            "q_network": {
            ...
            for i in range(num_agents):
                final_params_i = {
                    "actor": {
                        "params": final_agent_ts[i].actor_train_state.params,
                        "batch_stats": final_agent_ts[i].actor_train_state.batch_stats
                    },
                    "q_network": {
...
def run_training(config, wandb_logger, optimal_params, optimal_policies,
    ...
    joint_policies, init_joint_params = map(list, zip(*[
        initialize_reppo_agent(algorithm_config, env, init_rng, agent_index=i, observation_type=obs_type)
        for i in range(num_agents)
    ]))
...
def log_metrics(env, optimal_env, config, train_out, logger, metric_names: tuple, agent_idx: int):
    ...
    avg_per_agent_target_entropy_losses = []
    avg_per_agent_lagrangian_losses = []
    avg_per_agent_alpha = []
    avg_per_agent_lagrangian = []
    avg_per_agent_kl = []
    avg_per_agent_entropy = []
    avg_per_agent_critic_losses = []
    avg_per_agent_q_values = []
    avg_per_agent_q_error = []
    num_agents_log = sum(1 for k in train_metrics if k.endswith("/actor_loss") and k.startswith("agent_"))
    for i in range(num_agents_log):
        avg_per_agent_actor_losses.append(np.mean(np.asarray(train_metrics[f"agent_{i}/actor_loss"]), axis=(0, 2, 3))) # shape (n_train_seeds, num_updates, num_update_epochs, num_minibatches)
        avg_per_agent_target_entropy_losses.append(np.mean(np.asarray(train_metrics[f"agent_{i}/target_entropy_loss"]), axis=(0, 2, 3)))
        avg_per_agent_lagrangian_losses.append(np.mean(np.asarray(train_metrics[f"agent_{i}/lagrangian_loss"]), axis=(0, 2, 3)))
        avg_per_agent_alpha.append(np.mean(np.asarray(train_metrics[f"agent_{i}/alpha"]), axis=(0, 2, 3)))
        avg_per_agent_lagrangian.append(np.mean(np.asarray(train_metrics[f"agent_{i}/lagrangian"]), axis=(0, 2, 3))) # shape (n_train_seeds, num_updates, num_update_epochs, num_minibatches)
    ...
    num_updates = len(avg_per_agent_actor_losses[0])
    for step in range(num_updates):
        ...
        for i in range(num_agents_log):
            ...
            logger.log_item(f"{prefix}/TargetEntropyLoss", avg_per_agent_target_entropy_losses[i][step], train_step=step, commit=True)
            logger.log_item(f"{prefix}/LagrangianLoss", avg_per_agent_lagrangian_losses[i][step], train_step=step, commit=True)
            logger.log_item(f"{prefix}/Alpha", avg_per_agent_alpha[i][step], train_step=step, commit=True)
            logger.log_item(f"{prefix}/Lagrangian", avg_per_agent_lagrangian[i][step], train_step=step, commit=True)
    ...
    if env._render:
        ...
        if hasattr(eval_render_worst_case_env_state, 'state') and 'agent-at' in eval_render_worst_case_env_state.state:
            ...
            num_episodes = eval_render_worst_case_env_state.positions.shape[1]
    ...
    for i in range(num_agents_log):
        ...
        savepath = save_train_run(train_out_i, savedir, savename=f"REPPO_Agent_{agent_idx + 1}_Optimize_Train_Run-Agent_{i + 1}")
        out_savepaths.append(savepath)
        if config["logger"]["log_train_out"]:
            logger.log_artifact(name=f"REPPO_Agent_{agent_idx + 1}_Optimize_Train_Run-Agent_{i + 1}", path=savepath, type_name="joint_train_run")
