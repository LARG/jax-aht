...
def make_train(config, env):
    ...
    def train(rng):
        ...
        (obsv, obsv_full), env_state = jax.vmap(env.reset, in_axes=(0,))(reset_rng)
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                (new_obs, new_obs_full), new_env_state, reward, new_done, info = jax.vmap(env.step, in_axes=(0,0,0))(
...
def run_ippo(config, logger):
    ...
    log.info(f"Starting IPPO marl training ...")
    ...
    log.info(f"IPPO marl training completed in {elapsed_time:.2f}s")
    log.info(f"IPPO marl training completed in {int(hours):02d}h {int(minutes):02d}m {int(seconds):02d}s {milliseconds:03d}ms {microseconds:03d}µs")

    policy, _ = initialize_agent(algorithm_config["ACTOR_TYPE"], algorithm_config, env, jax.random.PRNGKey(0))

    log_metrics(config, out, logger)
    return out, env, policy
