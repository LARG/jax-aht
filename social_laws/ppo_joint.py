...
def train_ppo_joint_agents(config, env, optimal_env, train_rng,
    ...
    Args:
        ...
        joint_policies: list of AgentPolicy, policies for the agents
        init_joint_params: list of dict, initial parameters for the agents
        ppo_policies: list of AgentPolicy, PPO policies for the agents
        ppo_params: list of dict, PPO parameters for the agents
        vf_policies: list of AgentPolicy, value function policies for the agents
        vf_params: list of dict, value function parameters for the agents
    ...
    def make_ppo_joint_train(config):
        ...
        def train(rng, agent_idx, *packed_params):
            # Unpack: agent_ppo_params (num_agents,) then agent_vf_params (num_agents,)
            ...
            def _env_step(runner_state, unused):
                ...
                new_vf_hstates = []
                for i in range(num_agents):
                    vf_aa, new_vf_hs = _get_vf_restricted_avail_actions(
                    ...
                    new_vf_hstates.append(new_vf_hs)

                # All agents: action, value, log_prob
            ...
            def _make_update_minbatch(agent_i):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, target_v):
                        _, value, pi, _ = joint_policies[agent_i].get_action_value_policy(
                            params=params,
                            obs=traj_batch.obs,
    ...
    # vmap over seeds (axis 0) and pass all per-agent ppo/vf params unpacked
    in_axes = (0, None) + (0,) * num_agents + (0,) * num_agents
    out = jax.vmap(train_fn, in_axes=in_axes)(
        rngs, agent_idx, *ppo_params, *vf_params
    )
    return out
