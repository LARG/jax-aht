        ...
        def train(rng, agent_idx, agent_0_ppo_params, agent_1_ppo_params, agent_0_vf_params, agent_1_vf_params):
            ...
            def _env_step(runner_state, unused):
                ...
                transition_0 = Transition(
                    ...
                    obs=get_agent_data(prev_full_obs, 0) if config["JOINT_USE_FULL_OBS"] else get_agent_data(prev_obs, 0), # shape (num_envs, obs_dim)
                    info=info,
                ...
                transition_1 = Transition(
                    ...
                    obs=get_agent_data(prev_full_obs, 1) if config["JOINT_USE_FULL_OBS"] else get_agent_data(prev_obs, 1), # shape (num_envs, obs_dim)
                    info=info,
            ...
            def _update_step(update_runner_state, unused):
                ...
                _, last_val_0, _, _ = agent_0_policy.get_action_value_policy(
                    params=agent_0_train_state.params,
                    obs=obs_full_0 if config["JOINT_USE_FULL_OBS"] else obs_0,
                    done=done_0,
                ...
                _, last_val_1, _, _ = agent_1_policy.get_action_value_policy(
                    params=agent_1_train_state.params,
                    obs=obs_full_1 if config["JOINT_USE_FULL_OBS"] else obs_1,
                    done=done_1,
...
def run_training(config, wandb_logger, ppo_params, ppo_policies,
    ...
    # Initialize agent
    agent_0_policy, agent_0_init_params = initialize_agent(algorithm_config, env, init_rng, agent_index=0, observation_type="full" if algorithm_config["JOINT_USE_FULL_OBS"] else "agent")
    agent_1_policy, agent_1_init_params = initialize_agent(algorithm_config, env, init_rng, agent_index=1, observation_type="full" if algorithm_config["JOINT_USE_FULL_OBS"] else "agent")
