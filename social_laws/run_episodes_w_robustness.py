...
def run_single_episode(rng, env, agent_idx, agent_params, agent_policies,
                       ...
                       max_episode_steps, epsilon_optimal, agent_test_mode=False):
    # Reset the env.
    ...
    optimal_obs, optimal_env_state, optimal_reward, optimal_done, optimal_info = env.step(step_rng, init_env_state, optimal_env_act)
    ...
    worst_final_carry, _ = jax.lax.scan(
    ...
    optimal_final_carry, _ = jax.lax.scan(
...
def run_episodes(rng, env, agent_idx, agent_params, agent_policies,
                 ppo_params, ppo_policies, vf_params, vf_policies,
                 max_episode_steps, num_eps, epsilon_optimal, agent_test_mode=False):
    ...
    def scan_episode(carry, ep_rng):
        info = run_single_episode(
            ...
            ppo_params, ppo_policies, vf_params, vf_policies,
            max_episode_steps, epsilon_optimal, agent_test_mode
        )
        return carry, info
...
# ***********************************************************************
# Render version of the above functions, which runs episodes
# sequentially but does not use scan (since scan is not compatible
# with rendering).
# ***********************************************************************

def run_single_render_episode(rng, env, agent_idx, agent_params, agent_policies,
                       ppo_params, ppo_policies, vf_params, vf_policies,
                       max_episode_steps, epsilon_optimal, agent_test_mode=False):
    # Reset the env.
    rng, reset_rng = jax.random.split(rng)
    ...
    _, worst_stacked_carry = jax.lax.scan(
        worst_scan_step, worst_init_carry, None, length=max_episode_steps)
    ...
    _, optimal_stacked_carry = jax.lax.scan(
        optimal_scan_step, optimal_init_carry, None, length=max_episode_steps)

    # Return all step data (stacked_carry contains data for each timestep)
    # The final info in the last valid step contains the episode return via LogWrapper
    return (worst_stacked_carry, optimal_stacked_carry, init_env_state)

def run_render_episodes(rng, env, agent_idx, agent_params, agent_policies,
                 ppo_params, ppo_policies, vf_params, vf_policies,
                 max_episode_steps, num_eps, epsilon_optimal, agent_test_mode=False):
    '''Run num_eps episodes sequentially using scan.'''
    # Create episode-specific RNGs
    ...
    # Define scan function to run episodes sequentially
    def scan_episode(carry, ep_rng):
        all_out = run_single_render_episode(
            ep_rng, env, agent_idx, agent_params, agent_policies,
            ppo_params, ppo_policies, vf_params, vf_policies,
            max_episode_steps, epsilon_optimal, agent_test_mode
        )
