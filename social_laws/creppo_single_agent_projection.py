...
def train_creppo_agent(config, env, train_rng,
                    policy, init_params, agent_idx):
    '''
    Train CREPPO single agent projection using the given initial parameters.
    ...
    '''
    # ------------------------------
    # Build the CREPPO single agent projection training function
    # ------------------------------
    def make_creppo_train(config):
        ...
        def train(rng, agent_idx):
            ...
            target_train_state = CustomTrainState.create(
                apply_fn=policy.q_network.apply,
                params=deepcopy(init_params["params"]),
                batch_stats=deepcopy(init_params["batch_stats"]),
                tx=optax.set_to_zero(),
            )

            train_state = CReppoTrainState(
                timesteps=0,
                n_updates=0,
                grad_steps=0,
            ...
            def _env_step(runner_state, unused):
                ...
                act, _, pi, new_hstate = policy.get_action_value_policy(
                ...
                next_q, _ = policy.get_critic_out(
                    params=(train_state.q_network_train_state.params, train_state.q_network_train_state.batch_stats),
            ...
            def _update_step(update_runner_state, unused):
                ...
                next_pi, _ = policy.get_critic_out(
                    params=(train_state.q_network_train_state.params, train_state.q_network_train_state.batch_stats),
                    obs=traj_batch.next_obs[-1].reshape(1, config["NUM_CONTROLLED_ACTORS"], -1),
                    done=traj_batch.done[-1].reshape(1, config["NUM_CONTROLLED_ACTORS"], -1),
                    avail_actions=traj_batch.next_avail_actions[-1],
                    hstate=hstate,
                    rng=next_rng
                )
                ...
                train_state = train_state.replace(
                    target_train_state=train_state.target_train_state.replace(
    ...
    train_fn = make_creppo_train(config)
...
def run_training(config, wandb_logger, agent_idx=0):
    ...
    # Initialize agent
    policy, init_params = initialize_creppo_agent(algorithm_config, env, init_rng, agent_index=agent_idx)

    log.info(f"Starting single agent projection training for agent {agent_idx}...")
    start_time = time.perf_counter()

    # Run the training
    out = train_creppo_agent(
...
def log_metrics(env, config, train_out, logger, metric_names: tuple, agent_idx: int):
    ...
    all_agent_alpha_loss = np.asarray(train_metrics["alpha_loss"]) # shape (n_train_seeds, num_updates, num_update_epochs, num_minibatches)
    ...
    has_collisions = "returned_episode_collisions" in train_metrics["ckpt_eval_ep_last_info"]
    if has_collisions:
        ...
        all_ckpt_agent_collisions = all_ckpt_collisions[:, :, :, agent_idx] # shape (n_train_seeds, num_updates, num_eval_episodes)
        all_agent_collisions = all_collisions[:, :, :, agent_idx] # shape (n_train_seeds, num_updates, num_eval_episodes)
        average_ckpt_agent_collisions_per_iter = np.mean(all_ckpt_agent_collisions, axis=(0, 2)) # shape (num_updates,)
        average_agent_collisions_per_iter = np.mean(all_agent_collisions, axis=(0, 2)) # shape (num_updates,)
    ...
    average_agent_alpha_losses = np.mean(all_agent_alpha_loss, axis=(0, 2, 3))  # shape (num_updates,)
    ...
    num_updates = len(average_agent_alpha_losses)
    for step in range(num_updates):
        ...
        if has_collisions:
            logger.log_item(f"Eval/Agent_{agent_idx + 1}_Proj/Collisions", average_agent_collisions_per_iter[step], train_step=step, commit=True)
            logger.log_item(f"Eval/Agent_{agent_idx + 1}_Proj/CheckpointCollisions", average_ckpt_agent_collisions_per_iter[step], train_step=step, commit=True)
        logger.log_item(f"Train/Agent_{agent_idx + 1}_Proj/AlphaLoss", average_agent_alpha_losses[step], train_step=step, commit=True)
    ...
    out_savepath = save_train_run(train_out, savedir, savename=f"CREPPO_Agent_{agent_idx + 1}_Proj_Train_Run")
    if config["logger"]["log_train_out"]:
        logger.log_artifact(name=f"CREPPO_Agent_{agent_idx + 1}_Proj_Train_Run", path=out_savepath, type_name="single_agent_proj_train_run")
