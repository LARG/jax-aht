...
def run_single_episode(rng, env, optimal_env, agent_idx, agent_params, agent_policies,
    ...
    hstates = []
    for i in range(num_agents):
        act_i, hs_i = agent_policies[i].get_action(
    ...
    def worst_scan_step(carry, _):
        ...
        ep_ts, env_state, obs, rng, done, reward, act_onehot, hstates, vf_hstates, last_info = carry
        output = carry
    ...
    def optimal_scan_step(carry, _):
        def take_optimal_step(carry_step):
            ...
            obs_per = [get_agent_data(obs, i).reshape(1, 1, -1) for i in range(num_agents)]
