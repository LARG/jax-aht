...
def run_single_episode(rng, env, agent_params, agent_policy,
    ...
    (init_obs, init_obs_full), _, init_env_state = env.reset(reset_rng)
    ...
    obs, _, env_state, reward, done, info = env.step(step_rng, init_env_state, env_act)
    ...
    def scan_step(carry, _):
        def take_step(carry_step):
            ...
            env_act_onehot = {k: jax.nn.one_hot(env_act[env.agents[i]], env.action_space(env.agents[i]).n) for i, k in enumerate(env.agents)}
            obs_next, _, env_state_next, reward, done_next, info_next = env.step(step_rng, env_state, env_act)
