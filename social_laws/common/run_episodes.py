...
def run_episodes(rng, env, agent_idx, agent_param, agent_policy,
                 max_episode_steps, num_eps, agent_test_mode=False):
    ...
    def scan_episode(carry, ep_rng):
        info = run_single_episode(
            ...
            max_episode_steps, agent_test_mode
        )
        return carry, info
...
def run_single_render_episode(rng, env, agent_idx, agent_param, agent_policy,
                       max_episode_steps, agent_test_mode=False):
    # Reset the env.
    rng, reset_rng = jax.random.split(rng)
    init_obs, init_env_state = env.reset(reset_rng)
    ...
    # We'll use a scan to iterate steps until the episode is done.
    ep_ts = 1
    init_carry = (ep_ts, env_state, obs, rng, done, reward, env_act_onehot, hstate, info)
    def scan_step(carry, _):
        def take_step(carry_step):
            ep_ts, env_state, obs, rng, done, reward, act_onehot, hstate, last_info = carry_step
            # Get available actions for the agent from environment state
        ...
        ep_ts, env_state, obs, rng, done, reward, act_onehot, hstate, last_info = carry
        # Output current carry before potentially updating it
        output = carry
        new_carry = jax.lax.cond(
            done["__all__"],
            lambda curr_carry: curr_carry, # True fn
...
def run_render_episodes(rng, env, agent_idx, agent_param, agent_policy,
