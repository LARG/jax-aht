...
def run_single_episode(rng, env, optimal_env, agent_idx, agent_params, agent_policy,
    ...
    joint_init_obs = batchify(init_obs, env.agents, num_agents * 1) # shape (num_agents * num_envs, obs_dim)
    joint_init_full_obs = batchify(init_obs_full, env.agents, num_agents * 1)
    joint_init_done = batchify(init_done, env.agents, num_agents * 1).squeeze(axis=-1)

    # Stack restricted available actions for all agents
    joint_optimal_restricted_avail_actions = jnp.stack(optimal_restricted_avail_actions, axis=0).reshape(num_agents * 1, -1)

    # Get joint agent action using centralized policy
    joint_act, joint_hstate = agent_policy.get_action(
        params=(agent_params["params"], agent_params["batch_stats"]),
    ...
    env_act = unbatchify(joint_act, env.agents, 1, num_agents)
    ...
    worst_init_carry = (worst_ep_ts, env_state, obs, rng, done, reward, env_act_onehot,
                        joint_hstate, optimal_vf_hstates, info)
    def worst_scan_step(carry, _):
        def take_worst_step(carry_step):
            ep_ts, env_state, (obs, obs_full), rng, done, reward, act_onehot, joint_hstate, optimal_vf_hstates, last_info = carry_step
            ...
            # Centralized joint observation - concatenate all agents' observations
            joint_obs = batchify(obs, env.agents, num_agents * 1)
            joint_full_obs = batchify(obs_full, env.agents, num_agents * 1)
            joint_done = batchify(done, env.agents, num_agents * 1).squeeze(axis=-1)

            # Stack restricted available actions for all agents
            joint_opt_restricted_avail_actions = jnp.stack(opt_restricted_avail_actions, axis=0).reshape(num_agents * 1, -1)

            # Get joint action from centralized policy
            joint_act, joint_hstate_next = agent_policy.get_action(
                ...
                avail_actions=joint_opt_restricted_avail_actions,
                hstate=joint_hstate,
                rng=act_rng,
                aux_obs=None,
                env_state=env_state,
            ...
            return (ep_ts + 1, env_state_next, obs_next, rng, done_next, reward, env_act_onehot,
                    joint_hstate_next, new_opt_hstates, info_next)

        ep_ts, env_state, obs, rng, done, reward, act_onehot, joint_hstate, optimal_vf_hstates, last_info = carry
        output = carry
