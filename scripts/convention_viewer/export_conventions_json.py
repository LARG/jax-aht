...
def main() -> int:
    ...
    ap.add_argument(
        ...
        required=True,
        type=Path,
        help="features.csv from a --br-paired PD run",
    )
    ap.add_argument(
        "--clusters",
        type=Path,
    ...
    ap.add_argument(
        "--out", type=Path, default=Path(__file__).resolve().parent / "site"
    )
    args = ap.parse_args()
    ...
    scores = feats.get("mean_final_score", None)
    pairing = feats.get("pairing", None)
    points = [
        {
            "name": names[i],
