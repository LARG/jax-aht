...
def fetch_train_run_metrics_cached(
    ...
    print(
        f"  [wandb] downloading {target_artifact.name} ({target_artifact.size / 1e6:.0f} MB)"
    )
...
def _maybe_reduce_partner_metrics(metrics: dict) -> dict:
    ...
    *only* to the per-step scalar pushed through `io_callback` to wandb,
    ...
    if mask_field is None:
        ...
        print(
            "  [warn] reducing partner metrics without 'returned_episode' mask — using plain mean"
        )
...
def make_curve(
    values_2d: np.ndarray, total_env_steps: int, n_segments: int | None = None
) -> CurveData:
